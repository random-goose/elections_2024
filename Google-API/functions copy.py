...
def plot_party_spending_by_demographics(df, start_date, end_date, age_groups, genders, numerical_columns, states, top_n_parties=10):
    ...
    # Filter the DataFrame by selected states (if provided)
    if states:
    ...
    # Loop through the filtered data by state, party, and age group
    for _, row in df.iterrows():
        # Convert demographics to a list of dictionaries
        demographic_data = ast.literal_eval(row['demographics'])

        # Extract the party associated with the ad
        party = row['party']

        # Accumulate spending based on demographics and party
        for demo in demographic_data:
            if isinstance(demo, dict) and 'age' in demo and 'gender' in demo:
                if demo['age'] in age_groups and demo['gender'] in genders:  # Filter by age group and gender
                    percentage = demo['percentage']
                    for col in party_sums:
                        amount = row[col] * percentage  # Calculate spending based on percentage

                        # Sum spending for each party
                        if party in party_sums[col]:
                            party_sums[col][party] += amount
                        else:
                            party_sums[col][party] = amount
    ...
    # Group by party and sum the spending, then sort in descending order
    compiled_df = compiled_df.groupby('party', as_index=False)['sum_amount'].sum()
    compiled_df = compiled_df.sort_values(by='sum_amount', ascending=False)

    # Select the top N parties by spending
    top_parties_df = compiled_df.head(top_n_parties)

    # Plot the data for the top parties
    fig = px.bar(top_parties_df, x='party', y='sum_amount', color='party',
                 ...
                 color_discrete_sequence=px.colors.qualitative.Plotly,
                 category_orders={'party': top_parties_df['party'].tolist()})  # Order by sorted party list

    fig.write_html("top_parties_spending.html")
    # fig.show()

    # return {'count': len(df), 'party_sums': top_parties_df.to_dict()}
    return fig
